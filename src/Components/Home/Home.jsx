...
const cdata=useSelector(store=>store.citydata);
    useEffect(()=>{
        fetch("http://localhost:3000/add-city").then(Response=>Response.json()).then(data=>setData(data))
    },[])
   ...
   const countryfun=(e)=>{
      const cfdata=cdata.filter((elem)=>{
            return elem.country==e.target.value;
      })
       setData(cfdata)
   }
      ...
      return(
          <div>
              ...
              <div>
              <p className="country">Filter by Country</p>
              <select onChange={countryfun}>
              <option value=""></option>
